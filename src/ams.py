...
sol_weight = []
sol_label = []
with open("../Data/sol_file.csv","r") as file:
	reader = csv.reader(file,delimiter=",")
	for row in reader:
		sol_id.append(int(row[0]))
		sol_weight.append(float(row[1]))
		sol_label.append(row[2])
...
with open("../Data/preds_0_175.csv","r") as file:
	...
	for row in reader:
		...
		pred.append([int(row[0]),float(row[1]),row[2]])
pred = sorted(pred, key=lambda x:x[0])
pred = [pred[i] for i in range(100000,len(pred))]
sol_id = [sol_id[i] for i in range(350000,800000)]
sol_label = [sol_label[i] for i in range(350000,800000)]
sol_weight = [sol_weight[i] for i in range(350000,800000)]
