            ...
            for row in sub:
                if row[2] == 's': # only events predicted to be signal are scored
                    if solutionDict[row[0]][1] == 's':
                        signal += float(solutionDict[row[0]][0])/(464.338)
                    elif solutionDict[row[0]][1] == 'b':
                        background += float(solutionDict[row[0]][0])/(276507.96)
...
if __name__ == "__main__":
    ...
    path = ""
    solutionFile = "../Data/sol_file.csv"
    submissionFile = "../Data/final-submission-1.csv"
